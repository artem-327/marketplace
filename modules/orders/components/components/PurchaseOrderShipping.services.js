...
export const getInitialFormValues = (props) => {
  ...
  if (initialValues.pickupDate && moment(initialValues.pickupDate).endOf('day').isAfter(moment()))
    initialValues.pickupDate = moment(initialValues.pickupDate).endOf('day').format(getLocaleDateFormat())
  else initialValues.pickupDate = moment().endOf('day').format(getLocaleDateFormat())
    ...
    return Yup.object().shape({
      pickupDate: dateValidation(false).concat(
        Yup.string().test(
          ...
          val => moment('00:00:00', 'hh:mm:ss').diff(getStringISODate(val), 'days') <= 0
        )
      )
    })
  })

...
export default {
  ...
  getOrder: (endpointType, orderId) =>
    api.get(`/prodex/api/${endpointType}-orders/${orderId}`),
  ...
  searchCompany: companyText =>
    api.get(`/prodex/api/companies/search?pattern=${companyText}`),
  searchProducts: productText =>
    api.get(`/prodex/api/products/search?limit=30&search=${productText}`),
  getLots: poId => api.get(`/prodex/api/product-offers/${poId}`),
  assignLots: (orderId, orderItemId, assignedLots) =>
    api.patch(
      `/prodex/api/sale-orders/${orderId}/order-item/${orderItemId}/assign-lots`,
      assignedLots
    ),
  linkAttachment: (lotId, aId) =>
    api.post(
      `/prodex/api/attachment-links/to-lot?attachmentId=${aId}&lotId=${lotId}`
    ),
  ...
  payOrder: (orderId, bankAccount) =>
    api.post(
      `/prodex/api/payments/pay?purchaseOrderId=${orderId}&bankAccountId=${bankAccount}`
    ),
  ...
  approveOrder: orderId =>
    api.patch(`/prodex/api/purchase-orders/${orderId}/approve`),
  disapproveOrder: orderId =>
    api.patch(`/prodex/api/purchase-orders/${orderId}/disapprove`)
}

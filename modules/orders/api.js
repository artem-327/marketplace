...
export default {
  ...
  searchCompany: (companyText, returnWhenEmpty = true) => api.get(`/prodex/api/companies/search?pattern=${encodeURIComponent(companyText)}${returnWhenEmpty ? '&returnWhenEmpty=true' : ''}`),
  ...
  orderResolutionReopen: (orderId, ordersType) =>
    api
      ...
      .then(res => res.data)
      .catch(e => console.error(e)),
  downloadDisputeAttachment: (orderId, attachmentId) =>
    api.get(`/prodex/api/purchase-orders/${orderId}/attachments/download-dispute-attachment/${attachmentId}`, {
      responseType: 'blob'
    })
}

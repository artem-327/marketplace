...
function mapStateToProps(store, { datagrid }) {
  return {
    ...
    defaultZip: getSafe(
      () => store.auth.identity.homeBranch.deliveryAddress.address.zip.zip,
      ''
    ),
    rows: datagrid.rows.map(po => {
      const qtyPart = getSafe(
        () => po.companyProduct.packagingUnit.nameAbbreviation
      )
      return {
        ...po,
        id: po.id,
        productName: po.companyProduct.echoProduct.name,
        productNumber: getSafe(
          () => po.companyProduct.echoProduct.code,
          'Unmapped'
        ),
        // merchant: getSafe(() => po.warehouse.warehouseName, ''),
        available: po.pkgAvailable ? (
          <FormattedNumber minimumFractionDigits={0} value={po.pkgAvailable} />
        ) : (
          'N/A'
        ),
        packagingType: getSafe(() => po.companyProduct.packagingType.name, ''),
        packagingUnit: getSafe(
          () => po.companyProduct.packagingUnit.nameAbbreviation,
          ''
        ),
        packagingSize: getSafe(() => po.companyProduct.packagingSize, ''),
        quantity: qtyPart ? (
          <FormattedUnit unit={qtyPart} separator=' ' value={po.quantity} />
        ) : (
          'N/A'
        ),
        fobPrice:
        ...
        manufacturer: getSafe(
          () => po.companyProduct.echoProduct.manufacturer.name,
          'N/A'
        ),
...
export default withDatagrid(
  connect(
    mapStateToProps,
    { ...Actions, sidebarChanged, openPopup, getProductOffer, applyFilter }
  )(Marketplace)
)

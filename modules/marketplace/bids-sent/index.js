...
export const BidsSent = props => {
  ...
  const urlApiConfig = {
    url: gaSearch
      ? `/prodex/api/product-offer-bids/own/datagrid?${GA_TRACK_QUERY}=${encodeURIComponent(gaSearch)}`
      : `/prodex/api/product-offer-bids/own/datagrid`,
    searchToFilter: v => {
      setGaSearch(getSafe(() => v.searchInput, ''))
      let filters = { or: [], and: [], url: '' }
      if (v && v.searchInput) {
        filters.url = `/prodex/api/product-offer-bids/own/search?pattern=${encodeURIComponent(v.searchInput)}`
      }
      if (v && v.initId) {

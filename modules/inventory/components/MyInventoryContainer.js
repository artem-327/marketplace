...
import { openImportPopup } from "~/src/pages/settings/actions.js"
...
function mapStateToProps(store) {
  return {
    ...
    rows: store.simpleAdd.myProductOffers.map(po => {
      const qtyPart = `${
        po.product.packagingUnit
          ? po.product.packagingUnit.nameAbbreviation
          : ""
      }`
      ...
      return {
        ...
        packaging:
          po.product.packagingType && po.product.packagingType.name
            ? po.product.packagingType.name
            : "N/A",
        pkgAmount: qtyPart ? `${po.product.packagingSize} ${qtyPart}` : "N/A",
        quantity: qtyPart
          ? `${(
              parseInt(po.pkgAmount, 10) *
              parseInt(po.product.packagingSize, 10)
            ).formatNumber()} ${qtyPart}`
          : "N/A",
        ...
        fobPrice:
          po.pricingTiers.length > 1
            ...
            : "N/A",
        manufacturer:
          po.manufacturer && po.manufacturer.name

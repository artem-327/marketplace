...
import { Container, Menu, Header, Checkbox, Button } from "semantic-ui-react"
import SubMenu from "~/src/components/SubMenu"
import { FormattedMessage } from "react-intl"
import Router from "next/router"
import ProdexGrid from "~/components/table"
import ProductImportPopup from "~/src/pages/settings/components/ProductCatalogTable/ProductImportPopup.js"
    ...
    return rows.map(r => ({
      ...
      broadcast: (
        <div style={{ float: "right" }}>
          <Checkbox
            toggle
            checked={r.broadcasted}
            onChange={() => alert("Changed:" + r.productName)}
          />
        </div>
      )
    }))
      ...
      <>
            ...
            <Menu.Menu position="right">
              <Menu.Item>
                <Button
                  ...
                  primary
                  onClick={this.props.openImportPopup}
                >
        ...
        <ProdexGrid
          ...
          renderGroupLabel={({ row: { value } }) => {
            ...
            return (
              <span>
                <span style={{ color: "#2599d5" }}>{number}</span>&nbsp;&nbsp;{" "}
                {name} <span className="right">Product offerings: {count}</span>
              </span>
            )
          }}
          ...
          rowActions={[
            {
              text: "Edit listing",
              callback: row => Router.push(`/inventory/edit/${row.id}`)
            },

...
class LogisticsTable extends Component {
  ...
  render() {
    const {
      openPopup, logisticsAccounts, loading,
      intl: { formatMessage }, deleteLogisticsAccount, toastManager } = this.props
        ...
        },
        {
          ...
          callback: (row) => {
            confirm(
              ...
              formatMessage({ id: 'confirm.logisticsAccount.content', defaultMessage: `Do you really want to delete ${row.logisticsProviderName}?` }, { name: row.logisticsProviderName }))
              .then(async () => {
                try {
                  await deleteLogisticsAccount(row.id)
                  toastManager.add(generateToastMarkup(
                    formatMessage({ id: 'notifications.logisticsDeleted.header', defaultMessage: 'Logistics Account deleted' }),
                    formatMessage({ id: 'notifications.logisticsDeleted.content', defaultMessage: `Logistics Account ${row.logisticsProviderName} successfully deleted.` }, { name: row.logisticsProviderName })
                  ), { appearance: 'success' })
                } catch{ }
              })
              .catch(() => { })
...
const mapDispatchToProps = {
  openPopup, getLogisticsAccounts, deleteLogisticsAccount
}

const mapStateToProps = ({ settings: { loading, logisticsAccounts, deleteLogisticsAccount } }) => ({
  loading, logisticsAccounts, deleteLogisticsAccount
})

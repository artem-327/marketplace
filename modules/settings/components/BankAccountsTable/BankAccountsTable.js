...
const FinalizeConfirmDialog = confirmable(({ proceed, show, dismiss }) => (
  <Formik
    ...
    validationSchema={Yup.object().shape({
      amount1: Yup.number(errorMessages.mustBeNumber).required(
        errorMessages.requiredMessage
      ),
      amount2: Yup.number(errorMessages.mustBeNumber).required(
        errorMessages.requiredMessage
      )
    })}
      ...
      <Modal
        ...
        <Modal.Content>
          <Segment basic loading={isSubmitting}>
            <Form>
              ...
              <Form.Group
                widths='equal'
                data-test='settings_bank_account_amounts_inp'>
                <Input
                  label={
                    <FormattedMessage
                      id='settings.amountNum'
                      defaultMessage='Amount 1'
                      values={{ num: 1 }}
                    />
                  }
                  name='amount1'
        ...
        <Modal.Actions>
          <Button
            ...
            inverted
            onClick={handleReset}
            data-test='settings_bank_account_cancel_btn'>
          ...
          <Button
            ...
            data-test='settings_bank_account_confirm_btn'>
  ...
  render() {
    ...
    return (
      <React.Fragment>
            ...
            <Table style={{ marginTop: 30, marginBottom: 30 }}>
              ...
              <Table.Body>
                <Table.Row>
                  <Table.Cell>
                    <FormattedMessage
                      id={`dwolla.registrationStatus.${dwollaAccountStatus}`}
                    />
                  </Table.Cell>
                  <Table.Cell>
                    {bankAccounts.documentStatus ? (
                      <>
                        <FormattedMessage
                          id={`dwolla.info.${dwollaAccountStatus}`}
                        />
                        &nbsp;
                        <FormattedMessage
                          id={`dwolla.document.${dwollaDocumentRequired}`}
                        />
                      </>
                    ) : (
                      <FormattedMessage
        ...
        {bankAccounts.documentStatus && (
          <div>
            <FormattedMessage id='dwolla.document.explanatory.header1'>
              {text => <h3>{text}</h3>}
            </FormattedMessage>
            <FormattedMessage id='dwolla.document.explanatory.text1'>
              {text => <div>{text}</div>}
            </FormattedMessage>
            <FormattedMessage id='dwolla.document.explanatory.header2'>
              {text => <h3>{text}</h3>}
            </FormattedMessage>
            <FormattedMessage id='dwolla.document.explanatory.text21'>
              {text => <div>{text}</div>}
            </FormattedMessage>
            <br></br>
            <li>
              <FormattedMessage id='dwolla.document.explanatory.BoldLi11'>
                {text => <b>{text}</b>}
              </FormattedMessage>
            ...
            <li>
              <FormattedMessage id='dwolla.document.explanatory.BoldLi12'>
                {text => <b>{text}</b>}
              </FormattedMessage>
            ...
            <li>
              <FormattedMessage id='dwolla.document.explanatory.BoldLi13'>
                {text => <b>{text}</b>}
              </FormattedMessage>
            ...
            <FormattedMessage id='dwolla.document.explanatory.text22'>
              {text => <div>{text}</div>}
            </FormattedMessage>
            <br></br>
            <FormattedMessage id='dwolla.document.explanatory.li21'>
              {text => <li>{text}</li>}
            </FormattedMessage>
            <FormattedMessage id='dwolla.document.explanatory.li22'>
              {text => <li>{text}</li>}
            </FormattedMessage>
            <FormattedMessage id='dwolla.document.explanatory.li23'>
              {text => <li>{text}</li>}
            </FormattedMessage>
            <FormattedMessage id='dwolla.document.explanatory.li24'>
              {text => <li>{text}</li>}
            </FormattedMessage>
          </div>
        )}
...
const statusToLabel = {
  ...
  verification_in_process: (
    <Label color='orange' horizontal>
      <FormattedMessage
        id='settings.verificationInProcess'
...
const mapStateToProps = state => {
  ...
  let dwollaDocumentRequired =
    company && company.dwollaDocumentRequired
      ? company.dwollaDocumentRequired

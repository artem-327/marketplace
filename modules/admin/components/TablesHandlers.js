...
class TablesHandlers extends Component {
  ...
  render() {
    ...
    return (
      <Menu secondary>
        <Menu.Item header>
          <Header as='h1' size='medium'>{currentTab.name}</Header>
        </Menu.Item>
        {!currentTab.hideHandler &&
          <Menu.Menu position='right'>
            <Menu.Item data-test='admin_table_search_inp'>
              <Input style={{ width: 340 }} size='large' icon='search' placeholder={formatMessage({ id: config[currentTab.name].searchText })}
                onChange={(e, { value }) => {

  ...
  render() {
    ...
    return (
      <CompanyProductInfo
        ...
        header={<Modal.Header><FormattedMessage id={`global.${popupValues ? 'edit' : 'add'}`}>{(text) => text}</FormattedMessage> {config.addEditText}</Modal.Header>}
        ...
        handleSubmit={async (values, formikProps) => {
          ...
          toastManager.add(generateToastMarkup(
            ...
            <FormattedMessage id={`notifications.${status}.content`} values={{ name: values.casProduct.casIndexName }} />
          ), {
            appearance: 'success'
          })

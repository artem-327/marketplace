  ...
  render() {
    ...
    return (
      <FormField
        // error={errors[name]}
        name={name}>
        <ErrorLabel error={errors[name]}><FormattedMessage id='global.document' defaultMessage='Document' /></ErrorLabel>
        <Button icon as='label'>
          <FormattedMessage id='global.upload' defaultMessage='Upload'>{text => text}</FormattedMessage>
          {/* <Icon name='plus' /> */}
          <HiddenInput
            name={name}
            error={errors.file}

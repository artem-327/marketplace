  ...
  componentDidMount() {
    for (let i = 0; i < this.props.inputs.length; i++) {
      if (this.props.inputs[i].type === 'checkbox') {
        if (
          this.props.data[this.props.checkboxModel] &&
          this.props.data[this.props.checkboxModel][this.props.inputs[i].id]
        ) {
          this.props.onOpen(true)
        }
      } else {
        if (
          this.props.data[this.props.inputs[i].model.substring(1)] &&
          this.props.data[this.props.inputs[i].model.substring(1)] !== ''
        ) {
          this.props.onOpen(true)
        }
          ...
          switch (input.type) {
            ...
            case 'dropdown': {
              return (
                <div key={index} className='filter-input-dropdown'>
                  <label className='input-label' htmlFor={input.model}>
                    <FormattedMessage id={'filter.' + input.label.split(' ').join('')} defaultMessage={input.label} />
                  </label>
                  <DropdownRedux
                    clearable={true}
                    selection={true}
            ...
            case 'comboBox': {
              return (
                <div key={index} className='filter-input-dropdown'>
                  <label className='input-label' htmlFor={input.model}>
                    <FormattedMessage id={input.label.split(' ').join('')} defaultMessage={input.label + '1'} />
                  </label>
                  <ComboBoxRedux
                    dispatch={this.props.dispatch}
                    model={input.model}
                  ...
                  {this.props.searchedProducts && (
                    <datalist id='searched-products'>
                      {this.props.searchedProducts.map(product => (
                        <option value={product.name} label={product.casName} />
                      ))}
                    </datalist>
                  )}
                  {/* <RemoteComboBoxRedux
            ...
            case 'number': {
              return (
                <div key={index} className='filter-input-text'>
                  <label className='input-label' htmlFor={input.model}>
                    <FormattedMessage id={'filter.' + input.label} defaultMessage={input.label + '1'} />
                  </label>
                  <Control
                    component={Input}
                    validateOn='change'
            ...
            case 'assay': {
              ...
              return (
                <div key={index} className='filter-input-text'>
                  <label className='input-label' htmlFor={input.model}>
                    <FormattedMessage id={'filter.' + input.label} defaultMessage={input.label} />
                  </label>
                  <Control
                    component={Input}
                    type={input.type}
    ...
    return (
      <div className={classnames('filter-group', { split: this.props.split })}>
        <div
          ...
          <FormattedMessage id={'filter.' + this.props.header} defaultMessage={this.props.header} />
        </div>
        {this.renderInputs()}

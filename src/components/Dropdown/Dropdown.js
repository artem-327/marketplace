    ...
    return opt.map((option, index) => {
      return (
        <li
          key={index + 'dropdown'}
          onClick={() => {
            this.setCurrentValue(option.id, option.name || option.warehouseName)
          }}
          data-test={`Dropdown_index_${index}_set`}>
    ...
    return (
      <div className='dropdown-wr' ref={this.dropdownRef}>
        <div
          ...
          <div className='dropdown-current-placeholder'>
            {currentValue || this.props.placeholder || 'Select'}
            {arrow}
          </div>
        </div>
        {options}

...
class Rows extends Component {
    render() {
        let rows = this.props.rowsOpns.map((item, index) => {
            ...
            return this.props.data[item.index].rows.map((row, index2) => (<Row data={row} rowOpns={this.props.rowsOpns[item.index].rows[index2]} rowComponent={this.props.rowComponent} contextMenu={this.props.contextMenu} headers={this.props.headers} selectable={this.props.selectable} key={index2} groupId={item.index} id={item.id} selectFunc={this.props.selectFunc}/>)
            )
        });
        return (
            <tbody>

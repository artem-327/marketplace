...
class GroupRow extends Component {
    ...
    selectGroup(value){
        let rowsOpns = this.props.rowsOpns.map((r) => ({...r, selected: value}));
        this.props.selectGroup(this.props.index, rowsOpns)
    }

    isSelected(){
        for(let i = 0; i < this.props.rowsOpns.length; i++){
            if(!this.props.rowsOpns[i].selected) return false
        }
        return true
    }
        ...
        return (
            <React.Fragment>
                ...
                {this.state.open ? this.props.rowsOpns.rows.map((opns, index) => (
                    <Row selectable={this.props.selectable} id={opns.id} rowOpns={opns} rowComponent={this.props.rowComponent} contextMenu={this.props.contextMenu} headers={this.props.headers} data={this.props.rows.rows[opns.index]} selectFunc={this.props.selectFunc} groupId={opns.index} key={index}/>
                )) : null}
            </React.Fragment>
        )

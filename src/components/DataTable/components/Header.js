          ...
          {this.props.data.header.map((item, index) =>
            item.visible ? (
              <th
                ...
                <div className={'fix-header'}>
                  <FormattedMessage id={'dataTable.' + item.name} defaultMessage={item.name} />
                  {this.state.open[item.name] ? (
                    <ul className='data-table-context-th'>
                      {/*<li onClick={() => this.props.sortFunc(item.name)}>Sort</li>*/}
                      <li
                        onClick={() => this.props.toggleColumn(item.index, false)}
                        data-test={`table_header_row_index_${index}_toggle_column`}>
                        Hide
                      </li>
                    </ul>
                  ) : null}

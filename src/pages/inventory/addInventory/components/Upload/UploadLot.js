...
class UploadLot extends Component {
  ...
  removeFile = index => {
    if (this.state.files[index] instanceof Blob) {
      ...
      this.props
        .removeAttachmentLink(this.props.lot ? true : false, this.props.lot ? this.props.lot.id : poId, attachmentId)
        .then(() => {
      ...
      <div className={'uploadLot ' + this.props.className + (hasFile ? ' has-file' : '')}>
        ...
        {hasFile ? (
          ...
          <ReactDropzone className='dropzoneLot' activeClassName='active' onDrop={this.onPreviewDrop}>
            ...
            <div>
              <label>
                <FormattedMessage id='addInventory.uploadDocument' defaultMessage='Click to upload' /> {this.props.type}
              </label>
            </div>
          </ReactDropzone>
        )}

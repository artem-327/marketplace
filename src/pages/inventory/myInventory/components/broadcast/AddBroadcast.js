...
class AddBroadcast extends Component {
  ...
  initTable = (root, dispatch) => {
    ...
    regions.forEach(region => {
      if (region.anonymous === 1 || region.anonymous === 2)
        dispatch(actions.change(`forms.brcRules.region[${region.id}].anonymous`, true))
      if (region.anonymous === 2) {
        dispatch(actions.change(`forms.brcRules.region[${region.id}].anonymousPartly`, true))
      }

      if (region.broadcast === 1 || region.broadcast === 2)
        dispatch(actions.change(`forms.brcRules.region[${region.id}].broadcast`, true))
    ...
    flattenStates.forEach(state => {
      if (state.anonymous === 1 || state.anonymous === 2)
        dispatch(actions.change(`forms.brcRules.state[${state.id}].anonymous`, true))
      if (state.anonymous === 2) {
        dispatch(actions.change(`forms.brcRules.state[${state.id}].anonymousPartly`, true))
      }

      if (state.broadcast === 1 || state.broadcast === 2)
        dispatch(actions.change(`forms.brcRules.state[${state.id}].broadcast`, true))
    ...
    flattenCompanies.forEach(company => {
      if (company.anonymous === 1 || company.anonymous === 2)
        dispatch(actions.change(`forms.brcRules.company[${company.id}].anonymous`, true))
      if (company.anonymous === 2) {
        dispatch(actions.change(`forms.brcRules.company[${company.id}].anonymousPartly`, true))
      }

      if (company.broadcast === 1 || company.broadcast === 2)
        dispatch(actions.change(`forms.brcRules.company[${company.id}].broadcast`, true))
    ...
    flattenOffices.forEach(office => {
      if (office.anonymous === 1 || office.anonymous === 2)
        dispatch(actions.change(`forms.brcRules.office[${office.id}].anonymous`, true))
      if (office.anonymous === 2) {
        dispatch(actions.change(`forms.brcRules.office[${office.id}].anonymousPartly`, true))
      }
      if (office.broadcast === 1 || office.broadcast === 2)
        dispatch(actions.change(`forms.brcRules.office[${office.id}].broadcast`, true))
  ...
  convertDataForPost = async () => {
    ...
    const regionsPost = await Object.values(storedRegions).map(i => {
      ...
      const statesOfThisRegion = Object.values(storedStates).filter(obj =>
        parentRegion['elements'].find(obj2 => obj.id === obj2.id)
      )
      region.elements = statesOfThisRegion.map(i => {
        ...
        const companiesOfThisState = Object.values(storedCompanies).filter(obj =>
          parentState['elements'].find(obj2 => obj.id === obj2.id)
        )
        //COMPANIES
        state.elements = companiesOfThisState.map(i => {
          ...
          const officesOfThisState = Object.values(storedOffices).filter(obj =>
            parentStatesOffices.flat().find(obj2 => obj.id === obj2.id)
          )
  ...
  rulesUpdate = () => {
    ...
    if (clickedModel.includes('brcRules')) {
      if (clickedModel.includes('root')) {
        rules.forEach(rule => {
          if (clickedModel.includes(rule)) {
            ...
            broadcastRegions.forEach(
              i =>
                i[rule] !== isChecked && dispatch(actions.change(`forms.brcRules.region[${i.id}].${rule}`, isChecked))
            )
            broadcastStates.forEach(
              i => i[rule] !== isChecked && dispatch(actions.change(`forms.brcRules.state[${i.id}].${rule}`, isChecked))
            )
            broadcastCompanies.forEach(
              i =>
                i[rule] !== isChecked && dispatch(actions.change(`forms.brcRules.company[${i.id}].${rule}`, isChecked))
            )
            broadcastOffices.forEach(
              i =>
                i[rule] !== isChecked && dispatch(actions.change(`forms.brcRules.office[${i.id}].${rule}`, isChecked))
            )
            ...
            broadcastRegions.forEach(
              i =>
                i[rule] !== isChecked &&
                dispatch(actions.change(`forms.brcRules.region[${i.id}].${rule}Partly`, !isChecked))
            )
            broadcastStates.forEach(
              i =>
                i[rule] !== isChecked &&
                dispatch(actions.change(`forms.brcRules.state[${i.id}].${rule}Partly`, !isChecked))
            )
            broadcastCompanies.forEach(
              i =>
                i[rule] !== isChecked &&
                dispatch(actions.change(`forms.brcRules.company[${i.id}].${rule}Partly`, !isChecked))
            )
            broadcastOffices.forEach(
              i =>
                i[rule] !== isChecked &&
                dispatch(actions.change(`forms.brcRules.office[${i.id}].${rule}Partly`, !isChecked))
            )
          }
        })
        units.forEach(unit => {
          if (clickedModel.includes('priceUnit')) {
            if (root[1].priceUnit === unit) {
              ...
              broadcastCompanies.forEach(i =>
                dispatch(actions.change(`forms.brcRules.company.${i.id}.priceUnit`, unit))
              )
        ...
        if (clickedModel.includes('priceValue')) {
          if (root[1].priceValue) {
            broadcastRegions.forEach(i =>
              dispatch(actions.change(`forms.brcRules.region.${i.id}.priceValue`, root[1].priceValue))
            )
            broadcastStates.forEach(i =>
              dispatch(actions.change(`forms.brcRules.state.${i.id}.priceValue`, root[1].priceValue))
            )
            broadcastCompanies.forEach(i =>
              dispatch(actions.change(`forms.brcRules.company.${i.id}.priceValue`, root[1].priceValue))
            )
            broadcastOffices.forEach(i =>
              dispatch(actions.change(`forms.brcRules.office.${i.id}.priceValue`, root[1].priceValue))
            )
          }
          if (!root[1].priceValue) {
      ...
      if (clickedModel.includes('region')) {
        ...
        rules.forEach(rule => {
          if (clickedModel.includes(rule)) {
            ...
            if (broadcastRegions.some(region => region[rule] === true))
              dispatch(actions.change(`forms.brcRules.root[1].${rule}`, true))
            if (broadcastRegions.every(region => region[rule] !== true))
              dispatch(actions.change(`forms.brcRules.root[1].${rule}`, false))

            if (
              broadcastRegions.every(region => region[rule] === true) &&
              broadcastRegions.every(region => region[`${rule}Partly`] !== true)
            )
            ...
            if (clickedBroadcastRegion[rule] === isChecked && clickedRegion.elements) {
              clickedRegion.elements.forEach(state => {
                ...
                state.elements.forEach(company => {
                  ...
                  if (allOfficesBrc.every(obj => obj[rule] !== true))
                    company[rule] !== false &&
                      dispatch(actions.change(`forms.brcRules.company[${company.id}].${rule}`, false))
                  if (allOfficesBrc.some(obj => obj[rule] === true))
                    company[rule] !== true &&
                      dispatch(actions.change(`forms.brcRules.company[${company.id}].${rule}`, true))
                  if (allOfficesBrc.every(obj => obj[rule] === true))
                    company[`${rule}Partly`] !== false &&
                      dispatch(actions.change(`forms.brcRules.company[${company.id}].${rule}Partly`, false))
                  if (allOfficesBrc.some(obj => obj[rule] !== true))
                    company[`${rule}Partly`] !== true &&
                      dispatch(actions.change(`forms.brcRules.company[${company.id}].${rule}Partly`, true))
                  if (company.elements)
                    company.elements.forEach(office => {
                      office[rule] !== isChecked &&
        ...
        if (clickedModel.includes('priceUnit')) {
          units.forEach(unit => {
            if (broadcastRegions.every(i => i.priceUnit === unit))
              dispatch(actions.change(`forms.brcRules.root[1].priceUnit`, unit))
            if (!broadcastRegions.every(i => i.priceUnit === '$') && !broadcastRegions.every(i => i.priceUnit === '%'))
              dispatch(actions.change(`forms.brcRules.root[1].priceUnit`, ''))
            if (clickedBroadcastRegion && clickedBroadcastRegion.priceUnit === unit) {
              clickedRegion.elements.forEach(state => {
                ...
                state.elements.forEach(company => {
                  ...
                  const allOffices = allCompanies.map(i => i.elements).flat()
                  const allOfficesBrc = broadcastOffices.filter(obj => allOffices.find(obj2 => obj.id === obj2.id))
                  if (allOfficesBrc.every(i => i.priceUnit === unit))
        ...
        if (clickedModel.includes('priceValue')) {
          if (broadcastRegions.every(i => i.priceValue === broadcastRegions[0].priceValue))
            dispatch(actions.change(`forms.brcRules.root[1].priceValue`, clickedBroadcastRegion.priceValue))
          if (broadcastRegions.some(i => i.priceValue !== broadcastRegions[0].priceValue))
            dispatch(actions.change(`forms.brcRules.root[1].priceValue`, ''))
          clickedRegion.elements.forEach(state => {
            ...
            state.elements.forEach(company => {
              if (company.elements)
                company.elements.forEach(office => {
                  dispatch(
                    actions.change(`forms.brcRules.office[${office.id}].priceValue`, clickedBroadcastRegion.priceValue)
                  )
                })
              const allCompanies = flattenCompanies.filter(i => i.id === company.id)
              const allOffices = allCompanies.map(i => i.elements).flat()
              const allOfficesBrc = broadcastOffices.filter(obj => allOffices.find(obj2 => obj.id === obj2.id))
              if (allOfficesBrc.every(i => i.priceValue === allOfficesBrc[0].priceValue))
                dispatch(
                  actions.change(`forms.brcRules.company[${company.id}].priceValue`, clickedBroadcastRegion.priceValue)
                )
              if (!allOfficesBrc.every(i => i.priceValue === allOfficesBrc[0].priceValue))
                dispatch(actions.change(`forms.brcRules.company[${company.id}].priceValue`, ''))
            })
          })
        }
      }
      ...
      if (clickedModel.includes('state')) {
        ...
        rules.forEach(rule => {
          if (clickedModel.includes(rule)) {
            ...
            if (statesFiltered.every(state => state[rule] !== true))
              dispatch(actions.change(`forms.brcRules.region[${clickedState.regionId}].${rule}`, false))
            if (
              statesFiltered.every(state => state[rule] !== true) &&
              broadcastRegions.every(region => region[`${rule}`] !== true)
            )
              dispatch(actions.change(`forms.brcRules.root[1].${rule}`, false))
            if (statesFiltered.some(state => state[rule] !== true)) {
            ...
            if (
              statesFiltered.every(state => state[rule] === true) &&
              statesFiltered.every(state => state[`${rule}Partly`] !== true)
            )
              dispatch(actions.change(`forms.brcRules.region[${clickedState.regionId}].${rule}Partly`, false))
            if (
              statesFiltered.every(state => state[rule] === true) &&
              broadcastRegions.every(region => region[rule] === true) &&
              broadcastRegions.every(region => region[`${rule}Partly`] !== true)
        ...
        if (clickedModel.includes('priceUnit')) {
          units.forEach(unit => {
            if (statesFiltered.every(i => i.priceUnit === unit) && broadcastRegions.every(i => i.priceUnit === unit))
              dispatch(actions.change(`forms.brcRules.root[1].priceUnit`, unit))
            if (statesFiltered.every(i => i.priceUnit === unit))
              dispatch(actions.change(`forms.brcRules.region[${clickedState.regionId}].priceUnit`, unit))
          ...
          if (!statesFiltered.every(i => i.priceUnit === '$') && !statesFiltered.every(i => i.priceUnit === '%'))
            dispatch(actions.change(`forms.brcRules.region[${clickedState.regionId}].priceUnit`, ''))
          if (
        ...
        if (clickedModel.includes('priceValue')) {
          if (everyStatesValuesEqual && everyRegionsValuesEqual)
            dispatch(actions.change(`forms.brcRules.root[1].priceValue`, clickedBroadcastState.priceValue))
          if (everyStatesValuesEqual)
            dispatch(
              actions.change(
                `forms.brcRules.region[${clickedState.regionId}].priceValue`,
                clickedBroadcastState.priceValue
              )
            )
          if (someStatesValuesNotEqual || someRegionsValuesNotEqual)
            dispatch(actions.change(`forms.brcRules.root[1].priceValue`, ''))
          if (someStatesValuesNotEqual)
            dispatch(actions.change(`forms.brcRules.region[${clickedState.regionId}].priceValue`, ''))
          clickedState.elements.forEach(company => {
            if (company.elements)
              company.elements.forEach(office => {
                dispatch(
                  actions.change(`forms.brcRules.office[${office.id}].priceValue`, clickedBroadcastState.priceValue)
                )
              })
            const allCompanies = flattenCompanies.filter(i => i.id === company.id)
            const allOffices = allCompanies.map(i => i.elements).flat()
            ...
            if (allOfficesBrc.every(i => i.priceValue === allOfficesBrc[0].priceValue))
              dispatch(
                actions.change(`forms.brcRules.company[${company.id}].priceValue`, clickedBroadcastState.priceValue)
              )
            if (!allOfficesBrc.every(i => i.priceValue === allOfficesBrc[0].priceValue))
              dispatch(actions.change(`forms.brcRules.company[${company.id}].priceValue`, ''))
          })
      ...
      if (clickedModel.includes('company')) {
        ...
        rules.forEach(rule => {
          if (clickedModel.includes(rule)) {
            ...
            parentStates.forEach(state => {
              const companiesOfThisState = broadcastCompanies.filter(obj =>
                state['elements'].find(obj2 => obj.id === obj2.id)
              )
              if (
                companiesOfThisState.some(company => company[rule] !== true) &&
                !companiesOfThisState.every(company => company[rule] === true)
              ) {
              ...
              if (companiesOfThisState.every(company => company[rule] !== true)) {
                ...
                dispatch(actions.change(`forms.brcRules.state[${state.id}].${rule}`, true))
              }
            })

            parentRegions.forEach(region => {
              const statesOfThisRegion = broadcastStates.filter(obj =>
                region['elements'].find(obj2 => obj.id === obj2.id)
              )
              if (
                statesOfThisRegion.every(state => state[rule] === true) &&
                statesOfThisRegion.every(state => state[`${rule}Partly`] !== true)
              )
                dispatch(actions.change(`forms.brcRules.region[${region.id}].${rule}Partly`, false))
              if (
                statesOfThisRegion.some(state => state[rule] !== true) ||
                statesOfThisRegion.some(state => state[`${rule}Partly`] === true)
              ) {
              ...
              if (statesOfThisRegion.every(state => state[rule] !== true)) {
                ...
                dispatch(actions.change(`forms.brcRules.region[${region.id}].${rule}`, true))
              }
            })
        ...
        if (clickedModel.includes('priceValue')) {
          const everyCompaniesValuesEqual = broadcastCompanies.every(
            i => i.priceValue === broadcastCompanies[0].priceValue
          )
          ...
          parentStates.forEach(state => {
            ...
            if (companiesOfThisState.every(i => i.priceValue === companiesOfThisState[0].priceValue)) {
              dispatch(
                actions.change(`forms.brcRules.state.${state.id}.priceValue`, companiesOfThisState[0].priceValue)
              )
          ...
          parentRegions.forEach(region => {
            ...
            if (statesOfThisRegion.every(i => i.priceValue === statesOfThisRegion[0].priceValue)) {
              dispatch(
                actions.change(`forms.brcRules.region.${region.id}.priceValue`, statesOfThisRegion[0].priceValue)
              )
          ...
          allOffices.forEach(office => {
            dispatch(
              actions.change(`forms.brcRules.office[${office.id}].priceValue`, clickedBroadcastCompany.priceValue)
            )
          })
        }
      }
      ...
      if (clickedModel.includes('office')) {
        ...
        const officesCompanyFiltered = broadcastOffices.filter(obj =>
          parentCompanyOffices.flat().find(obj2 => obj.id === obj2.id)
        )
        const parentState = flattenStates.find(state => state.id === clickedOffice.stateId)
        const parentStatesOffices = parentState.elements.map(i => i.elements)
        const officesFiltered = broadcastOffices.filter(obj =>
          parentStatesOffices.flat().find(obj2 => obj.id === obj2.id)
        )
        ...
        rules.forEach(rule => {
          if (clickedModel.includes(rule)) {
            ...
            if (officesCompanyFiltered.some(office => office[rule] === true))
              dispatch(actions.change(`forms.brcRules.company[${clickedOffice.companyId}].${rule}`, true))
            if (officesCompanyFiltered.every(office => office[rule] !== true))
              dispatch(actions.change(`forms.brcRules.company[${clickedOffice.companyId}].${rule}`, false))
            if (officesCompanyFiltered.some(office => office[rule] !== true))
              dispatch(actions.change(`forms.brcRules.company[${clickedOffice.companyId}].${rule}Partly`, true))
            if (officesCompanyFiltered.every(office => office[rule] === true))
              dispatch(actions.change(`forms.brcRules.company[${clickedOffice.companyId}].${rule}Partly`, false))

            if (officesFiltered.every(office => office[rule] !== true))
              dispatch(actions.change(`forms.brcRules.state[${clickedOffice.stateId}].${rule}`, false))
            if (
              officesFiltered.every(office => office[rule] !== true) &&
              statesFiltered.every(state => state[rule] !== true)
            )
              dispatch(actions.change(`forms.brcRules.region[${clickedOffice.regionId}].${rule}`, false))
            ...
            if (officesFiltered.every(office => office[rule] === true))
              dispatch(actions.change(`forms.brcRules.state[${clickedOffice.stateId}].${rule}Partly`, false))
            if (
              officesFiltered.every(office => office[rule] === true) &&
              statesFiltered.every(state => state[`${rule}Partly`] !== true)
            )
              dispatch(actions.change(`forms.brcRules.region[${clickedOffice.regionId}].${rule}Partly`, false))
          }
        })
        ...
        if (clickedModel.includes('priceUnit')) {
          units.forEach(unit => {
            ...
            if (officesUnitsEqual && statesUnitsEqual && regionsUnitsEqual)
              dispatch(actions.change(`forms.brcRules.root[1].priceUnit`, unit))
            if (officesUnitsEqual && statesUnitsEqual)
              dispatch(actions.change(`forms.brcRules.region[${clickedOffice.regionId}].priceUnit`, unit))
            if (officesUnitsEqual)
              dispatch(actions.change(`forms.brcRules.state[${clickedOffice.stateId}].priceUnit`, unit))
            if (officesCompanyUnitsEqual)
              dispatch(actions.change(`forms.brcRules.company[${clickedOffice.companyId}].priceUnit`, unit))
          })
          if (
            (!officesFiltered.every(i => i.priceUnit === '$') && !officesFiltered.every(i => i.priceUnit === '%')) ||
            (!statesFiltered.every(i => i.priceUnit === '$') && !statesFiltered.every(i => i.priceUnit === '%'))
          ) {
          ...
          if (
            !officesCompanyFiltered.every(i => i.priceUnit === '$') &&
            !officesCompanyFiltered.every(i => i.priceUnit === '%')
          ) {
        ...
        if (clickedModel.includes('priceValue')) {
          const officesValuesNotEqual =
            officesFiltered && officesFiltered.some(i => i.priceValue !== officesFiltered[0].priceValue)
          const statesValuesNotEqual =
            statesFiltered && statesFiltered.some(i => i.priceValue !== statesFiltered[0].priceValue)
          const regionsValuesNotEqual =
            broadcastRegions && broadcastRegions.some(i => i.priceValue !== broadcastRegions[0].priceValue)
          const officesValuesEqual =
            officesFiltered && officesFiltered.every(i => i.priceValue === officesFiltered[0].priceValue)
          const statesValuesEqual =
            statesFiltered && statesFiltered.every(i => i.priceValue === statesFiltered[0].priceValue)
          const regionsValuesEqual =
            broadcastRegions && broadcastRegions.every(i => i.priceValue === broadcastRegions[0].priceValue)

          const officesValuesCompanyNotEqual =
            officesCompanyFiltered &&
            officesCompanyFiltered.some(i => i.priceValue !== officesCompanyFiltered[0].priceValue)
          const officesValuesCompanyEqual =
            officesCompanyFiltered &&
            officesCompanyFiltered.every(i => i.priceValue === officesCompanyFiltered[0].priceValue)
          ...
          if (officesValuesEqual && statesValuesEqual)
            dispatch(
              actions.change(
                `forms.brcRules.region[${clickedOffice.regionId}].priceValue`,
                clickedBroadcastOffice.priceValue
              )
            )
          if (officesValuesEqual)
            dispatch(
              actions.change(
                `forms.brcRules.state[${clickedOffice.stateId}].priceValue`,
                clickedBroadcastOffice.priceValue
              )
            )
          if (officesValuesNotEqual || statesValuesNotEqual || regionsValuesNotEqual)
            dispatch(actions.change(`forms.brcRules.root[1].priceValue`, ''))
          if (officesValuesNotEqual || statesValuesNotEqual)
            dispatch(actions.change(`forms.brcRules.region[${clickedOffice.regionId}].priceValue`, ''))
          if (officesValuesNotEqual)
            dispatch(actions.change(`forms.brcRules.state[${clickedOffice.stateId}].priceValue`, ''))
          if (officesValuesCompanyNotEqual)
            dispatch(actions.change(`forms.brcRules.company[${clickedOffice.companyId}].priceValue`, ''))
          if (officesValuesCompanyEqual)
            dispatch(
              actions.change(
                `forms.brcRules.company[${clickedOffice.companyId}].priceValue`,
                clickedBroadcastOffice.priceValue
              )
            )
        }
                    ...
                    <div className='add-broadcast'>
                      <div className='broadcast-nav'>
                        <div
                          ...
                          data-test='my_inventory_add_broadcast_switchToList'>
                          Client List
                        </div>
                        <div
                          className={!isClientList ? 'active' : ''}
                  ...
                  <GridColumn>
                    <Suspense fallback={<Spinner />}>
                      <CustomSegment secondary icon={true}>
                        <BroadcastingNumbers
                          storedOffices={storedOffices}
                          convertObjectToArray={this.convertObjectToArray}
                        />
                      </CustomSegment>
                    </Suspense>
                  </GridColumn>
          ...
          <div className='broadcast-filter'>
            ...
            <div className='group-item-wr smaller'>
              ...
              <Dropdown
                opns={[]}
                placeholder='Select Template'
                disabled={templatesOptions.length === 0 ? true : false}
                data-test='my_inventory_broadcast_template_drpdn'
              />
            </div>
          </div>

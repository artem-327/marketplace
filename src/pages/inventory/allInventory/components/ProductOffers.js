    ...
    let rows = Object.values(this.groupProductOffers(this.props.productOffers)).map(product => {
      return {
        ...
        rows: product.productOffers.map(offer => {
          ...
          const price =
            offer.pricingTiers.length > 1
              ? '$' +
                offer.pricingTiers[offer.pricingTiers.length - 1].price.formatMoney(3) +
                ' - ' +
                '$' +
                offer.pricingTiers[0].price.formatMoney(3)
          ...
          const location =
            this.props.identity.id === offer.owner.id || this.props.identity.homeBranch.id === offer.owner.id
              ? `${offer.warehouse.address.city}, ${offer.warehouse.address.country.name}`
              : `${offer.warehouse.address.country.name}` +
                (typeof offer.warehouse.address.country !== 'undefined'
                  ? `, ${offer.warehouse.address.country.name}`
                  : ``)
          ...
          return {
            ...
            data: [
              ...
              unit
                ? `${(
                    parseInt(offer.pkgAmount, 10) * parseInt(offer.product.packagingSize, 10)
                  ).formatNumber()} ${unit}`
                : 'N/A',
              price,
              offer.name,

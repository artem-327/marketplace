...
class Nav extends Component {
  ...
  renderDropdown(id, links, name) {
    ...
    const dropdown = (
      <div className='dropdown-nav-inside'>
        {links.map((link, index) => (
          <NavLink key={index} to={link.url} className='dropdown-nav-item' activeClassName='active'>
            <FormattedMessage id={'nav.dropdown.' + link.name.split(' ').join('')} defaultMessage={link.name} />
          </NavLink>
        ))}
      </div>
    )
  ...
  renderMenuItem(id, link, name) {
    ...
    return (
      <div className={'dropdown-nav ' + activeClass}>
        <NavLink to={id === 'dashboard' ? '/' : '/' + id} activeClassName='active'>
          <span className='dropdown-link-center'>
            <FormattedMessage id={'nav.' + name} defaultMessage={name} />
          </span>
        </NavLink>
  ...
  renderMyAccount(id, links) {
    const activeClass = this.props.location.pathname.split('/')[1] === id || this.state.dropdown[id] ? 'active' : null
    const dropdown = (
      <div className='dropdown-nav-inside-myaccount'>
        {links.map((link, index) =>
          link.name === 'Logout' ? (
            <div
              key={index}
              ...
              <FormattedMessage id={'nav.' + link.name} defaultMessage={link.name} />
            </div>
          ) : (
            <NavLink key={index} to={link.url} className='dropdown-nav-item' activeClassName='active'>
              {link.name}
    ...
    return (
      <div
        ...
        onClick={() => this.openDropdown(id)}
        data-test='Nav_dropdown_open_dropdown'>
        <span className='dropdown-link-center'>
          <img src={myAccount} alt='Logout'></img>
        </span>
        {this.state.dropdown[id] ? dropdown : null}
      </div>
    )

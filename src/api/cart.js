...
const api = {
  ...
  addCartItem: ({ productOffer, pkgAmount }) => axios.post('/prodex/api/cart/items', { productOffer, pkgAmount }).then(response => response.data),
  deleteCartItem: (cartItemId) => axios.delete(`/prodex/api/cart/items/${cartItemId}`),
  updateCartItem: ({ cartItemId, pkgAmount }) => axios.patch(`/prodex/api/cart/items/${cartItemId}?pkgAmount=${pkgAmount}`).then(response => response.data),
  ...
  postNewDeliveryAddress: (address) => axios.post('/prodex/api/delivery-addresses', address).then(response => response.data),
  updateDeliveryAddress: (address) => axios.put(`/prodex/api/delivery-addresses/id/${address.id}`, address).then(response => response.data), 
